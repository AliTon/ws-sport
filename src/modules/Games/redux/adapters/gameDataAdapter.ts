...
export default function gameDataAdapter(
    data: IMatch[]
): Record<string, ISport> {
    ...
    for (const item of data) {
        ...
        if (!result[sportId].regions[regionId]) {
            result[sportId].regions[regionId] = {
                ...item.region,
                matchesCount: 0,
            }
        }
        ...
        if (!result[sportId].regions[regionId].tournaments[tournamentId]) {
            result[sportId].regions[regionId].tournaments[tournamentId] = {
                ...item.tournament,
                matches: {},
                matchesCount: 0,
            }
        ...
        if (
            ...
            result[sportId].regions[regionId].tournaments[tournamentId].matches[
                matchId
            ] = { ...item }
        }
    }
